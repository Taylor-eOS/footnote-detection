...
def _snap_to_whitespace_bounds(text,start,end):
    n=len(text)
    ...
    while i<n:
        if text[i].isdigit():
            ...
            while j<n and text[j].isdigit():
                j+=1
            num=text[i:j]
            start=max(0,i-CONTEXT_JSON)
            end=min(n,j+CONTEXT_JSON)
...
def save_entries(entries,path=OUTPUT_PATH):
    clean=[{"number":e["number"],"context":e["context"],"label":e["label"]} for e in entries]
